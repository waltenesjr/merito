...
class Application_Model_DbTable_TabelaAvaliacoes extends Zend_Db_Table
{
    ...
    public function atualizar($dados)
    {
        ...
        try {
            ...
            $where = $this->getAdapter()->quoteInto('tabela_avaliacoes_id = ?', $dados['tabela_avaliacoes_id']);
            
            unset($dados['tabela_avaliacoes_id']);
    ...
    public function excluir($dados)
    {
        ...
        try {
            ...
            $where = $this->getAdapter()->quoteInto('tabela_avaliacoes_id = ?', $dados['id']);
        ...
        try{
            $colunas = array(
                ...
                'nota' => new Zend_Db_Expr('(SELECT a1.tabela_avaliacoes_nota 
                                        FROM '.$this->_schema.'.tabela_avaliacoes AS a1 
                                        WHERE a1.fk_profissionais_id = '.$parametros["fk_profissionais_id"].' 
                                            ...
                                            order by a1.tabela_avaliacoes_id desc limit 1)'),

                'id' => new Zend_Db_Expr('(SELECT a1.tabela_avaliacoes_id 
                                        FROM '.$this->_schema.'.tabela_avaliacoes AS a1 
                                        WHERE a1.fk_profissionais_id = '.$parametros["fk_profissionais_id"].' 
                                            AND a1.fk_atribuicoes_id = b.fk_atribuicoes_id
                                            AND TO_CHAR(a1.tabela_avaliacoes_timestamp, \'YYYY-MM-DD\') = \''.$parametros["tabela_avaliacoes_timestamp"].'\'
    ...
    public function getRegistros($colunas = array(),$parametros = array())
    {
        ...
        try{
            ...
            if(sizeof($parametros) > 0){
                //id
                if(isset($parametros["tabela_avaliacoes_id"]) && $parametros["tabela_avaliacoes_id"] > 0 )
                    $select->where("tabela_avaliacoes_id = ?", $parametros["tabela_avaliacoes_id"]);
                //descricao
                ...
                if(isset($parametros["orderBy"])){
                    ...
                    $select->order("tabela_avaliacoes_id DESC");
                }
            }
    ...
    public function getParametroPeloId($tabela_avaliacoes_id, $parametro = 'tabela_avaliacoes_descricao'){
        try{
            $select = $this
                ->select()
                ->setIntegrityCheck(false)
                ...
                ->where("tabela_avaliacoes_status != 'Inativo'")
                ->where("tabela_avaliacoes_id = ?",$tabela_avaliacoes_id);
    ...
    public function getId($parametros){
        try{
            $select = $this
                ->select()
                ->setIntegrityCheck(false)
                ->from(array('a'=>$this->_name), array('a.tabela_avaliacoes_id'), $this->_schema)
        ...
        if(isset($retorno) && sizeof($retorno) > 0)
            return $retorno[0]['tabela_avaliacoes_id'];
        else
            return 0;
    }
    ...
    public function getUltimosInseridos($parametros){
        try{
            $select = $this
                ...
                ->order("tabela_avaliacoes_id DESC");
	...
	try{
		...
		->from($this->_name, array(new Zend_Db_Expr("min(tabela_avaliacoes_nota) as min_nota"),
							       ...
							       new Zend_Db_Expr("TO_CHAR(tabela_avaliacoes_timestamp, 'MM') as mes"),
							       new Zend_Db_Expr("TO_CHAR(tabela_avaliacoes_timestamp, 'YYYY') as ano"),
							       new Zend_Db_Expr("TO_CHAR(tabela_avaliacoes_timestamp, 'DD-MM-YYYY') as fecha")
								   ), $this->_schema)
		->where("fk_profissionais_id = ?",$parametros['fk_profissionais_id'])
		...
		->group("TO_CHAR(tabela_avaliacoes_timestamp, 'YYYY')")
		->group("TO_CHAR(tabela_avaliacoes_timestamp, 'DD-MM-YYYY')")
